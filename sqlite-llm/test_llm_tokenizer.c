...
static void test_tokenize(sqlite3 *db) {
    ...
    snprintf(sql, sizeof(sql), "SELECT llm_tokenize('hello', '%s')", json_path);
    ...
    /* Should be BLOB type */
    CHECK(sqlite3_column_type(stmt, 0) == SQLITE_BLOB, "result is BLOB");

    {
        int nbytes = sqlite3_column_bytes(stmt, 0);
        const int32_t *ids = (const int32_t *)sqlite3_column_blob(stmt, 0);
        int count = nbytes / (int)sizeof(int32_t);

        /* "hello" should merge to a single token 7 */
        CHECK(count == 1, "tokenize 'hello' -> 1 token");
        if (count >= 1) {
            CHECK(ids[0] == 7, "tokenize 'hello' -> id 7");
        }
    }
    sqlite3_finalize(stmt);

    /* Test "he" -> token 5 */
    snprintf(sql, sizeof(sql), "SELECT llm_tokenize('he', '%s')", json_path);
    rc = sqlite3_prepare_v2(db, sql, -1, &stmt, NULL);
    ...
    CHECK(rc == SQLITE_ROW, "step llm_tokenize 'he'");
    {
        ...
        const int32_t *ids = (const int32_t *)sqlite3_column_blob(stmt, 0);
        int count = nbytes / (int)sizeof(int32_t);
        CHECK(count == 1, "tokenize 'he' -> 1 token");
        if (count >= 1) {
            CHECK(ids[0] == 5, "tokenize 'he' -> id 5");
        }
    }
    sqlite3_finalize(stmt);
}
...
static void test_detokenize(sqlite3 *db) {
    ...
    rc = sqlite3_prepare_v2(
        db, "SELECT llm_detokenize(X'07000000')", -1, &stmt, NULL);
    CHECK(rc == SQLITE_OK, "prepare llm_detokenize");
    rc = sqlite3_step(stmt);
    CHECK(rc == SQLITE_ROW, "step llm_detokenize");
    CHECK(sqlite3_column_type(stmt, 0) == SQLITE_TEXT, "result is TEXT");
    {
    ...
    rc = sqlite3_prepare_v2(db,
                            ...
                            &stmt,
                            NULL);
    CHECK(rc == SQLITE_OK, "prepare detokenize [5,6,4]");
    rc = sqlite3_step(stmt);
    CHECK(rc == SQLITE_ROW, "step detokenize [5,6,4]");
    {
        const char *text = (const char *)sqlite3_column_text(stmt, 0);
        CHECK(text != NULL && strcmp(text, "hello") == 0,
    ...
    rc = sqlite3_prepare_v2(
        db, "SELECT llm_detokenize(X'08000000')", -1, &stmt, NULL);
    CHECK(rc == SQLITE_OK, "prepare detokenize byte-bpe");
    rc = sqlite3_step(stmt);
    CHECK(rc == SQLITE_ROW, "step detokenize byte-bpe");
    {
        const char *text = (const char *)sqlite3_column_text(stmt, 0);
        CHECK(text != NULL && strcmp(text, " hello") == 0,
    ...
    rc = sqlite3_prepare_v2(db,
                            ...
                            &stmt,
                            NULL);
    CHECK(rc == SQLITE_OK, "prepare detokenize [0,7]");
    rc = sqlite3_step(stmt);
    CHECK(rc == SQLITE_ROW, "step detokenize [0,7]");
    {
        const char *text = (const char *)sqlite3_column_text(stmt, 0);
        CHECK(text != NULL && strcmp(text, "hello") == 0,
...
static void test_token_count(sqlite3 *db) {
    ...
    rc = sqlite3_prepare_v2(db,
                            ...
                            &stmt,
                            NULL);
    CHECK(rc == SQLITE_OK, "prepare llm_token_count");
    rc = sqlite3_step(stmt);
    CHECK(rc == SQLITE_ROW, "step llm_token_count");
    CHECK(sqlite3_column_int(stmt, 0) == 3, "token_count([5,6,4]) == 3");
    sqlite3_finalize(stmt);
}
...
static void test_token_at(sqlite3 *db) {
    ...
    rc = sqlite3_prepare_v2(db,
                            ...
                            &stmt,
                            NULL);
    CHECK(rc == SQLITE_OK, "prepare llm_token_at idx 0");
    rc = sqlite3_step(stmt);
    CHECK(rc == SQLITE_ROW, "step llm_token_at idx 0");
    CHECK(sqlite3_column_int(stmt, 0) == 5, "token_at([5,6,4], 0) == 5");
    sqlite3_finalize(stmt);
    ...
    rc = sqlite3_prepare_v2(db,
                            ...
                            &stmt,
                            NULL);
    CHECK(rc == SQLITE_OK, "prepare llm_token_at idx 2");
    rc = sqlite3_step(stmt);
    CHECK(rc == SQLITE_ROW, "step llm_token_at idx 2");
    CHECK(sqlite3_column_int(stmt, 0) == 4, "token_at([5,6,4], 2) == 4");
    sqlite3_finalize(stmt);
    ...
    rc = sqlite3_prepare_v2(db,
                            ...
                            &stmt,
                            NULL);
    CHECK(rc == SQLITE_OK, "prepare llm_token_at out-of-bounds");
    rc = sqlite3_step(stmt);
    CHECK(rc == SQLITE_ROW, "step llm_token_at out-of-bounds");
...
static void test_round_trip(sqlite3 *db) {
    ...
    rc = sqlite3_prepare_v2(db, sql, -1, &stmt, NULL);
    CHECK(rc == SQLITE_OK, "prepare round-trip");
    rc = sqlite3_step(stmt);
    CHECK(rc == SQLITE_ROW, "step round-trip");
    {
        const char *text = (const char *)sqlite3_column_text(stmt, 0);
        CHECK(text != NULL && strcmp(text, "hello") == 0,
