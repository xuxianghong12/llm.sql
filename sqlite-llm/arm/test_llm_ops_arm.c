...
static sqlite3 *open_test_db_arm(void) {
    ...
    rc = sqlite3_load_extension(
        db, "./llm_ops_arm.so", "sqlite3_llm_ops_init", &errmsg);
    if (rc != SQLITE_OK) {
        const char *msg = errmsg ? errmsg : sqlite3_errmsg(db);
...
static void test_nd_and_int8_smoke_arm(sqlite3 *db) {
    static const char *smoke_sqls[][2] = {
        ...
        {"llm_bool_to_additive_mask_nd_arm",
...
int main(void) {
    ...
    TestCase tests[] = {
        ...
        {"test_unary_activation_reduction_arm",
         test_unary_activation_reduction_arm},
    ...
    BenchmarkCase benches[] = {
        {"bench_llm_add_simd_arm",
         "SELECT llm_add_simd(?, ?)",
         500,
         2,
         bench_args},
        {"bench_llm_dot_simd_arm",
         "SELECT llm_dot_simd(?, ?)",
         1000,
         2,
         bench_args},
        {"bench_llm_mul_simd_arm",
         "SELECT llm_mul_simd(?, ?)",
         500,
         2,
         bench_args},
        {"bench_llm_div_simd_arm",
         "SELECT llm_div_simd(?, ?)",
         500,
         2,
         bench_args},
